...
def process(row, final_class):
    ...
    ax.set_title(f"{name} — {row['strong_line']} mask")
    ax = axes[1]
    ...
    ax.set_title(f"{row['strong_line']} profile")
    ...
    ax.step(v_h, specs[1], where="mid", color="C1", lw=0.7, label="skirt")
    ax.axvspan(v_cen - w_used / 2, v_cen + w_used / 2, alpha=0.15, color="C3")
    ax.axhline(0, color="gray", lw=0.5)
    ax.set_xlim(v_h[ch_lo], v_h[ch_hi])
    ax.set_title(f"{row['weak_line']} spectrum")
    ax.set_xlabel("v [km/s]"); ax.set_ylabel("S [Jy]")
