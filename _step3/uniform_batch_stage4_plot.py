...
Plotting convention (final review 2026-08-26, U-1 — supersedes the original
2026-07-10 prescription of flux+1σ for every source): detections/marginals as
markers with 1σ bars; upper limits as hollow markers AT 3σ_F with a downward
arrow and no error bar of their own. H30α and H40α get distinct hue+shape
(colorblind triple-coding rule). x-axis apportioning:
F_TIR(central) = F_TIR(IRAS global) × min(cov_corr, 1); covered sources → 1;
the thesis panel further scales by the mask-level 8um fraction (Option B).
...
def draw(ax, rows, xkey, ykey_scale):
    for r in rows:
        ...
        if tier in ("det", "marg"):
            # per-source offsets: NGC 4945/253/3628 crowd one spot (L11 2026-08-28)
            offs = {"ngc4945": (-64, -3), "ngc253": (6, -13),
                    "ngc3628": (5, 9)}.get(r["galaxy"], (6, 5))
            ax.annotate(display_name(r["galaxy"]), (x, y),
                        textcoords="offset points",
                        xytext=offs, fontsize=7,
...
def main():
    ...
    # Vertical offsets from the Bittner 2022 fit for the sources above the
    # upper-limit tier (calibrated S/N >= 3: the two detections and the three
    # marginal sources).  ch5, ch6 and ch7 quote mean, scatter, below-count
    # and the two extremes for BOTH axis conventions; printed here so a rerun
    # re-derives what the text claims (2026-08-27: NGC 4826 left the quoted
    # statistic when its per-source discussion was removed from the thesis;
    # the legacy six-source line is kept for the audit trail).
    def _offsets(rows_, xval):
        out = []
        for r in rows_:
            y = line_lum_conv(r) * N6[r["line"]] * r["F"]
            out.append((np.log10(y) - (HIST_FIT[0] * np.log10(xval(r))
                                       + HIST_FIT[1]), r["galaxy"]))
        return out

    five = sorted([r for r in brows if r["sn"] >= 3], key=lambda r: -r["sn"])
    legacy6 = sorted([r for r in brows if r["sn"] >= 2.5],
                     key=lambda r: -r["sn"])[:6]
    for tag, xf in (("aperture-matched (thesis axis)", lambda r: r["L_tir"]),
                    ("field-of-view axis", lambda r: (r["fir_central"]
                     * 4 * np.pi * (r["D"] * MPC_M) ** 2 / LSUN_W))):
        offs = _offsets(five, xf)
        v = np.array([o for o, _ in offs])
        print(f"\noffsets vs Bittner 2022 fit, {tag}, "
              f"{len(offs)} sources above the upper-limit tier:")
        for o, g in offs:
            print(f"   {g:<16}{o:+.2f} dex")
        lo, hi = min(offs), max(offs)
        print(f"   mean {v.mean():+.2f} | sd {v.std(ddof=1):.2f} | "
              f"below fit {int((v < 0).sum())}/{len(v)} | "
              f"highest {hi[0]:+.2f} ({hi[1]}) lowest {lo[0]:+.2f} ({lo[1]})")
    v6 = np.array([o for o, _ in _offsets(legacy6, lambda r: r["L_tir"])])
    print(f"   [audit] legacy six-source thesis-axis mean {v6.mean():+.2f} | "
          f"sd {v6.std(ddof=1):.2f} | below {int((v6 < 0).sum())}/6")
